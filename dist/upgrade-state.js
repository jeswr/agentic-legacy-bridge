 ...
 */
export function initialRelationship(personIri, currentChannel = "email", now = new Date()) {
 ...
 */
export function transition(state, event, now = new Date()) {
    ...
    switch (event.kind) {
        ...
        case "offer": {
            ...
            if (from !== "card-discovered" && from !== "aborted")
                return illegal(event, from);
        ...
        case "offer-response": {
            if (from !== "offer-pending" || state.pendingOffer === undefined)
                return illegal(event, from);
        ...
        case "revoke-verification": {
            ...
            if (from === "legacy-only" || from === "bridge-detected")
                return illegal(event, from);
            return {
                ok: true,
 ...
 */
export function assertRelationshipInvariant(state) {
    ...
    const floorAvailable = state.currentChannel === "email" || CHANNEL_ABOVE_FLOOR.has(state.currentChannel);
    if (!floorAvailable) {
        throw new Error("invariant: the email floor is not available.");
    }
    const needsVerified = new Set([
        "identity-verified",
        "card-discovered",
        "offer-pending",
        "upgraded",
...
export async function serializeRelationship(state, resourceUrl) {
    ...
    store.addQuad(subject, namedNode(AGENTIC_RELATIONSHIP_STATE), namedNode(stateNameToIri(state.state)));
    ...
    if (state.verifiedWebId !== undefined) {
        ...
        if (w !== undefined)
            store.addQuad(subject, namedNode(AGENTIC_VERIFIED_WEB_ID), namedNode(w));
    }
    if (state.agentCardUrl !== undefined) {
        const c = safeHttpIri(state.agentCardUrl);
        if (c !== undefined)
            store.addQuad(subject, namedNode(AGENTIC_AGENT_CARD), namedNode(c));
    }
    if (state.pendingOffer !== undefined) {
        store.addQuad(subject, namedNode(AGENTIC_OFFERED_CHANNEL), literal(state.pendingOffer.targetChannel));
    ...
    if (state.abortReason !== undefined) {
        store.addQuad(subject, namedNode(DCT_DESCRIPTION), literal(sanitizeText(state.abortReason).slice(0, 512)));
    }
    if (state.updatedAt !== undefined) {
        store.addQuad(subject, namedNode(AGENTIC_UPDATED_AT), literal(state.updatedAt, namedNode(XSD_DATE_TIME)));
    }
 ...
 */
export function parseRelationship(turtle, resourceUrl) {
    const safeResource = safeHttpIri(resourceUrl);
    if (safeResource === undefined)
        return undefined;
...
//# sourceMappingURL=upgrade-state.js.map