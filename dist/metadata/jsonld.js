...
import { RDF_TYPE, SCHEMA, SCHEMA_DATE_SENT, SCHEMA_END_TIME, SCHEMA_EVENT, SCHEMA_EVENT_CANCELLED, SCHEMA_EVENT_SCHEDULED, SCHEMA_EVENT_STATUS, SCHEMA_LOCATION, SCHEMA_MESSAGE, SCHEMA_NAME, SCHEMA_START_TIME, SCHEMA_URL, } from "../vocab.js";
import { AMBIGUOUS_TZ_NOTE, asBoundedString, firstProp, parseWhen, prop, whenDatatype, } from "./values.js";
...
export function isSchemaOrgContext(context, depth = 0) {
    if (typeof context === "string")
        return SCHEMA_CONTEXTS.has(context.trim());
...
function nodeTypes(node) {
    ...
    if (Array.isArray(raw)) {
        return raw.slice(0, 8).filter((t) => typeof t === "string");
    }
    return [];
}
/** True when a node declares the given schema.org type (bare / prefixed / full-IRI). */
export function hasSchemaType(node, typeName) {
...
export function isAgenticReplyNode(node) {
...
function emit(ctx, subject, predicate, object, overrides) {
    ctx.out.push({
        subject,
        predicate,
        object,
...
function emitWhen(ctx, subject, predicate, raw) {
    const when = parseWhen(raw);
    if (when === undefined)
        return;
    if (when.ambiguous) {
        emit(ctx, subject, predicate, { kind: "literal", value: when.value, datatype: whenDatatype(when) }, { confidence: 0.6, calibration: "SelfReported", note: AMBIGUOUS_TZ_NOTE });
        return;
    }
 ...
 */
export function mapEventNode(node, eventIri, ctx) {
    ...
    if (name !== undefined)
        emit(ctx, eventIri, SCHEMA_NAME, { kind: "literal", value: name });
    // location: a string, or a Place-shaped node with a `name` — stored as a plain literal.
    const locationRaw = prop(node, "location");
    const location = asBoundedString(locationRaw, MAX_LOCATION_CHARS) ??
    ...
    if (url !== undefined)
        emit(ctx, eventIri, SCHEMA_URL, { kind: "iri", value: url });
...
function topLevelNodes(root) {
    if (Array.isArray(root))
        return root.slice(0, MAX_TOP_LEVEL_NODES);
    const graph = prop(root, "@graph");
    if (Array.isArray(graph))
        return graph.slice(0, MAX_TOP_LEVEL_NODES);
    return [root];
}
/**
 ...
 */
export function extractJsonLdInterpretations(blocks, ctx) {
    const out = [];
    if (blocks === undefined || blocks.length === 0)
        return out;
    const emitCtx = { out, confidence: 1, calibration: "Calibrated" };
    let eventCount = 0;
    let messageCount = 0;
    for (const text of blocks) {
        ...
        if (isAgenticReplyNode(root))
            continue;
        // Closed-world context gate: only the well-known schema.org context is mapped.
        if (!isSchemaOrgContext(prop(root, "@context") ?? prop(Array.isArray(root) ? root[0] : root, "@context"))) {
            continue;
        }
        for (const node of topLevelNodes(root)) {
            if (eventCount >= MAX_EVENTS_PER_MESSAGE)
                break;
            ...
            if (hasSchemaType(node, "Event")) {
                ...
                if (hasSchemaType(wrapped, "Event"))
                    eventNode = wrapped;
            }
            ...
            if (hasSchemaType(node, "Message")) {
                if (messageCount >= MAX_EVENTS_PER_MESSAGE)
                    continue;
                messageCount++;
