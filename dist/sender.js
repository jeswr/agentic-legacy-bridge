...
import { asUrn, base64Url, mintUrn, normalizeEmailAddress, safeHttpIri, safeMailtoIri, sanitizeText, } from "./safe-iri.js";
import { AGENTIC_CANDIDATE_PERSON, AGENTIC_CANDIDATE_WEB_ID, AGENTIC_DKIM_DOMAIN_CLAIM, AGENTIC_IDENTITY_STATUS, FOAF_MBOX, FOAF_NAME, FOAF_PERSON, RDF_TYPE, SCHEMA_EMAIL, SCHEMA_IDENTIFIER, SCHEMA_NAME, SCHEMA_PERSON, VCARD_FN, VCARD_HAS_EMAIL, VCARD_INDIVIDUAL, } from "./vocab.js";
 ...
 */
export function addSenderPerson(store, message, options = {}) {
    const m = asBridgeMessage(message);
    ...
    for (const raw of options.candidateWebIds ?? []) {
        ...
        if (safe === undefined || seenWebIds.has(safe))
            continue;
        seenWebIds.add(safe);
    ...
    for (const raw of options.candidatePersonIris ?? []) {
        ...
        if (safe === undefined || safe === personIri || seenPersons.has(safe))
            continue;
 ...
 */
function asCandidatePersonIri(raw) {
    const urn = asUrn(raw);
    if (urn !== undefined) {
