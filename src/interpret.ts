...
export class DeterministicInterpreter implements Interpreter {
  interpret(message: EmailMessage, ctx: InterpretContext): Interpretation[] {
    ...
    // --- meeting times ---
    const times: Array<{ iso: string; confidence: number; calibrated: boolean; note?: string }> =
      [];
    for (const iso of extractIsoDateTimes(text)) {
      times.push({ iso, confidence: 0.95, calibrated: true });
      if (times.length >= MAX_EVENTS) break;
    }
    ...
    const eventName =
      message.subject !== undefined && message.subject.trim() !== ""
...
export const deterministicInterpreter: Interpreter = new DeterministicInterpreter();
 ...
 */
export function extractIsoDateTimes(text: string): string[] {
  ...
  while ((m = Re.exec(text)) !== null && count < 128) {
    ...
    if (!sameInstant(iso, back)) continue;
    if (!seen.has(back)) {
      seen.add(back);
      out.push(back);
    }
 ...
 */
export function extractRelativeMeetings(
  text: string,
  now: Date,
): Array<{ iso: string; note: string }> {
  const Re =
    /\b(today|tomorrow|(?:next\s+)?(sunday|monday|tuesday|wednesday|thursday|friday|saturday))\b[^.\n]{0,40}?\bat\s+(\d{1,2})(?::(\d{2}))?\s*(am|pm)?/gi;
  const out: Array<{ iso: string; note: string }> = [];
  const seen = new Set<string>();
  let m: RegExpExecArray | null;
  let count = 0;
  // biome-ignore lint/suspicious/noAssignInExpressions: standard exec loop.
  while ((m = Re.exec(text)) !== null && count < 32) {
    ...
    if (dayWord === "today") dayOffset = 0;
    else if (dayWord === "tomorrow") dayOffset = 1;
    else if (weekday in WEEKDAYS) {
      const target = WEEKDAYS[weekday] as number;
      const cur = base.getUTCDay();
      let delta = (target - cur + 7) % 7;
      if (dayWord.startsWith("next") && delta === 0) delta = 7;
      dayOffset = delta;
    }
...
function detectPolarity(text: string): { value: string; confidence: number } | undefined {
