...
import { buildAgenticGraph, type InterpretationStatus } from "./graph.js";
import {
  assertNoRedirect,
  assertWritableUrl,
  messageSlug,
...
import { type ExtractionTask, type LlmExtractor, LlmInterpreter } from "./interpret-llm.js";
 ...
 */
export function reparseRawAnchor(options: ReparseRawAnchorOptions): BridgeMessage | undefined {
  ...
  try {
    switch (channel) {
      ...
      case "whatsapp": {
        ...
        if (delivery.total === 0 || delivery.capped) return undefined;
        for (let i = 0; i < delivery.total; i++) {
          let candidate: BridgeMessage;
          try {
            candidate = delivery.messageAt(i);
          } catch (err) {
            if (err instanceof WhatsAppParseError) continue; // a non-text entry — skip it
            throw err;
          }
...
export type SweepAuditEvent =
  ...
  | { readonly kind: "interpreted"; readonly channel: string }
...
/** A GET of a pod RDF resource: the parsed dataset + its strong validator (ETag). */
 ...
 */
export async function sweepPendingInterpretations(
  options: SweepPendingInterpretationsOptions,
): Promise<SweepResult> {
  const container = canonicalContainer(options.container);
  if (container === undefined) {
    throw new Error(
      "sweep: container must be a safe http(s) container IRI ending in '/' with no query or fragment.",
    );
  }
  ...
  const graphsContainer =
    options.graphsContainer !== undefined ? canonicalContainer(options.graphsContainer) : container;
  if (graphsContainer === undefined) {
    throw new Error(
      "sweep: graphsContainer must be a safe http(s) container IRI ending in '/' with no query or fragment.",
    );
  }
  const readFetch = options.readFetch;
  const writeFetch = options.writeFetch ?? options.readFetch;
  const maxResources =
    Number.isInteger(options.maxResourcesPerSweep) && (options.maxResourcesPerSweep as number) > 0
      ? (options.maxResourcesPerSweep as number)
  ...
  const maxRawAnchorBytes =
    Number.isInteger(options.maxRawAnchorBytes) && (options.maxRawAnchorBytes as number) > 0
      ? (options.maxRawAnchorBytes as number)
      : DEFAULT_MAX_RAW_ANCHOR_BYTES;
  const maxGraphBytes =
    Number.isInteger(options.maxGraphBytes) && (options.maxGraphBytes as number) > 0
      ? (options.maxGraphBytes as number)
  ...
  const emit = (event: SweepAuditEvent): void => {
    try {
  ...
  for (const docUrl of candidates) {
    if (budgetUsed >= maxResources) break; // budget reached — the rest wait for next tick
    tally.examined++;
...
/** The discriminated outcome of processing one candidate resource. */
type ProcessOutcome =
  ...
  | { readonly kind: "retry"; readonly attempts: number }
  | { readonly kind: "failed"; readonly attempts: number }
  /** A CAS write-failure / thrown write error AFTER the model ran — budget-consuming. */
  | { readonly kind: "write-failed" };

interface ProcessContext {
  readonly options: SweepPendingInterpretationsOptions;
  readonly readFetch: typeof globalThis.fetch;
  readonly writeFetch: typeof globalThis.fetch;
  readonly cap: number;
  readonly maxRawAnchorBytes: number;
  readonly maxGraphBytes: number;
  readonly emit: (event: SweepAuditEvent) => void;
  /** The inbox container the raw anchors must live within (the sweep only READS here). */
  readonly inbox: string;
  /** The graphs container the graph resources live in + are CAS-written to. */
  readonly graphsContainer: string;
}

/** Process one candidate `.ttl` graph resource fail-closed. */
async function processResource(docUrl: string, ctx: ProcessContext): Promise<ProcessOutcome> {
  ...
  if (anchors.length !== 1) return { kind: "not-pending" };
  const anchor = anchors[0];
  if (anchor === undefined) return { kind: "not-pending" };
  ...
  if (status !== AGENTIC_PENDING) return { kind: "not-pending" };
  ...
  if (etag === null) return { kind: "skipped", reason: "no-etag" };
  if (!isStrongEtag(etag)) return { kind: "skipped", reason: "no-strong-etag" };
  ...
  if (channel === undefined) return { kind: "skipped", reason: "missing-channel" };

  const storedMediaType = safeMediaType(soleLiteral(dataset, anchor, AGENTIC_RAW_MEDIA_TYPE));
  if (storedMediaType === undefined) return { kind: "skipped", reason: "missing-media-type" };

  const recordedDigest = normalizeDigest(soleLiteral(dataset, anchor, AGENTIC_RAW_DIGEST));
  if (recordedDigest === undefined) return { kind: "skipped", reason: "missing-digest" };
  ...
  if (slug === undefined) return { kind: "skipped", reason: "bad-slug" };
  const recoveredId = slugToMessageId(slug);
  if (recoveredId === undefined) return { kind: "skipped", reason: "bad-slug" };
  ...
  if (urlQuads.length > 0) {
    ...
    if (derived !== undefined && isWithinBase(derived, ctx.inbox)) rawUrl = derived;
  }
  if (rawUrl === undefined) return { kind: "skipped", reason: "unsafe-raw-url" };

  const rawBytes = await fetchRawBytes(ctx.readFetch, rawUrl, ctx.maxRawAnchorBytes);
  if (rawBytes === undefined) return { kind: "skipped", reason: "raw-fetch-failed" };
  ...
  if (message === undefined) return { kind: "skipped", reason: "reconstruct-failed" };
  ...
  if (message.rawSha256 !== recordedDigest) return { kind: "skipped", reason: "digest-mismatch" };
...
/** The interpret + build + CAS-write stage (extracted so the caller wraps it budget-consumingly). */
interface InterpretAndWriteArgs {
  readonly safeDoc: string;
  readonly etag: string;
  readonly channel: string;
  readonly message: BridgeMessage;
  readonly dataset: DatasetCore;
  readonly anchor: Term;
  readonly storedMediaType: string;
  readonly rawUrl: string;
  readonly receivedAt: string | undefined;
  readonly interpretNow: Date;
  readonly attempts: number;
}

async function interpretAndWrite(
  ctx: ProcessContext,
  a: InterpretAndWriteArgs,
): Promise<ProcessOutcome> {
  const { safeDoc, etag, channel, message, dataset, anchor, interpretNow, attempts } = a;
  const { storedMediaType, rawUrl, receivedAt } = a;
  const det = (ctx.options.deterministicInterpreter ?? deterministicInterpreter).interpret(
    message,
    {
      docIri: safeDoc,
      now: interpretNow,
  ...
  let interpretations: readonly Interpretation[];
  let nextStatus: InterpretationStatus;
  let nextAttempts: number | undefined;
  let resultKind: "interpreted" | "retry" | "failed";
  if (clean) {
  ...
  const candidateWebIds = collectCandidateWebIds(
    dataset,
    anchor,
    ctx.options.candidateWebIdsFor?.(message),
  );
  ...
  if (put === "conflict") return { kind: "conflict" };
  if (put === "failed") return { kind: "write-failed" }; // post-model → budget-consuming

  if (resultKind === "interpreted") return { kind: "interpreted", channel };
  if (resultKind === "failed") return { kind: "failed", attempts: nextAttempts as number };
  return { kind: "retry", attempts: nextAttempts as number };
}
 ...
 */
async function readCappedBody(res: Response, maxBytes: number): Promise<Uint8Array | undefined> {
  const declared = Number(res.headers.get("content-length"));
  if (Number.isFinite(declared) && declared > maxBytes) return undefined;
  const body = res.body;
  if (body === null) return new Uint8Array(0); // a bodyless response — nothing to read
...
/** The three CAS outcomes of the conditional graph replace. */
type CasOutcome = "written" | "conflict" | "failed";
 ...
 */
function graphResourceCandidates(dataset: DatasetCore, container: string): string[] {
  const out: string[] = [];
  ...
  for (const quad of dataset.match(null, namedNode(LDP_CONTAINS), null, null)) {
    ...
    if (!safe.endsWith(".ttl") || safe.endsWith(".chat.ttl")) continue;
    if (!isWithinBase(safe, container)) continue;
...
function subjectsOfType(dataset: DatasetCore, typeIri: string): Term[] {
  const out: Term[] = [];
  const seen = new Set<string>();
 ...
 */
function soleObjectIri(dataset: DatasetCore, subject: Term, predicate: string): string | undefined {
  const quads = [...dataset.match(subject, namedNode(predicate), null, null)];
  if (quads.length !== 1) return undefined;
...
function soleLiteral(dataset: DatasetCore, subject: Term, predicate: string): string | undefined {
  const quads = [...dataset.match(subject, namedNode(predicate), null, null)];
  if (quads.length !== 1) return undefined;
  const obj = quads[0]?.object;
 ...
 */
function readAttempts(dataset: DatasetCore, subject: Term): number {
  const quads = [...dataset.match(subject, namedNode(AGENTIC_INTERPRETATION_ATTEMPTS), null, null)];
  if (quads.length !== 1) return 0; // missing or multiple ⇒ untrustworthy
  const obj = quads[0]?.object;
  if (obj === undefined || obj.termType !== "Literal") return 0;
  if (obj.datatype.value !== XSD_INTEGER) return 0; // MUST be a real xsd:integer
  if (!/^(?:0|[1-9]\d*)$/.test(obj.value)) return 0; // canonical lexical form only
 ...
 */
function collectCandidateWebIds(
  dataset: DatasetCore,
  anchor: Term,
  ...
  for (const senderQuad of dataset.match(anchor, namedNode(SCHEMA_SENDER), null, null)) {
    for (const webidQuad of dataset.match(
      senderQuad.object,
      namedNode(AGENTIC_CANDIDATE_WEB_ID),
      null,
      null,
    )) {
  ...
  for (const value of supplied ?? []) add(value);
  return out;
}
...
function slugFromDocUrl(docUrl: string): string | undefined {
  if (!docUrl.endsWith(".ttl")) return undefined;
  let path: string;
  try {
