...
interface PartCtx {
  parts: number;
  w: Warnings;
  /** Collected `text/calendar` part texts (count/size-capped, control-stripped). */
  calendars: string[];
  /** Collected `application/ld+json` part texts (count/size-capped, control-stripped). */
  jsonLd: string[];
}
 ...
 */
export function extractJsonLdScripts(html: string, collected: string[], w: Warnings): void {
  ...
  while (i < capped.length && collected.length < MAX_JSONLD_BLOCKS) {
    ...
    if (tagEnd === -1) break; // unterminated open tag → nothing further can be a block
    i = tagEnd + 1;
    ...
    if (close === -1) break; // unterminated block → drop (never guess at a boundary)
 ...
 */
function trimTrailingSpaceTab(line: string): string {
  let end = line.length;
  while (end > 0) {
...
export function parseEmail(input: string | Uint8Array): EmailMessage {
  ...
  const ctx: PartCtx = { parts: 0, w, calendars: [], jsonLd: [] };
