...
const ACTION_TYPES: ReadonlyMap<string, string> = new Map(
  (
    [
 ...
 */
export type AgenticReplyVerifier = (
  credential: Record<string, unknown>,
) => Promise<AgenticReplyVerification> | AgenticReplyVerification;
...
function readConformances(subject: unknown): PatternConformance[] {
  ...
  for (const entry of entries) {
    ...
    if (out.length >= MAX_CONFORMANCES) break;
  }
  return out;
}

/** The SYNC per-block mapping core (verification verdict already decided). */
function mapReplyBlock(
  root: Record<string, unknown>,
  ctx: InterpretContext,
  replyIndex: number,
  verified: boolean,
  issuer: string | undefined,
): { interpretations: Interpretation[]; patterns: PatternConformance[] } {
  const interpretations: Interpretation[] = [];
  const patterns: PatternConformance[] = [];
  ...
  const calibration: Interpretation["calibration"] = verified ? "Calibrated" : "SelfReported";
  ...
  const typeNames: unknown[] =
    typeof typeRaw === "string" ? [typeRaw] : Array.isArray(typeRaw) ? typeRaw.slice(0, 8) : [];
  ...
  const inReplyTo = asUrn(
    firstProp(subject, ["inReplyTo", "agentic:inReplyTo", AGENTIC_IN_REPLY_TO]),
  );
...
function replyRoots(blocks: readonly string[] | undefined): Record<string, unknown>[] {
  const out: Record<string, unknown>[] = [];
  if (blocks === undefined) return out;
  for (const text of blocks) {
    if (out.length >= MAX_REPLY_BLOCKS) break;
    const root = parseObject(text);
    if (root !== undefined && isAgenticReplyNode(root)) out.push(root);
  }
  return out;
}
...
function mergeExtractions(
  parts: ReadonlyArray<{ interpretations: Interpretation[]; patterns: PatternConformance[] }>,
  verified: boolean,
  issuer: string | undefined,
): AgenticReplyExtraction {
  const interpretations: Interpretation[] = [];
  const patterns: PatternConformance[] = [];
  const seen = new Set<string>();
 ...
 */
export function extractAgenticReplyStructural(
  blocks: readonly string[] | undefined,
  ctx: InterpretContext,
): AgenticReplyExtraction {
  const parts = replyRoots(blocks).map((root, i) =>
    mapReplyBlock(root, ctx, i + 1, false, undefined),
  );
 ...
 */
export async function extractAgenticReply(
  blocks: readonly string[] | undefined,
  ctx: InterpretContext,
  options?: { readonly verify?: AgenticReplyVerifier },
): Promise<AgenticReplyExtraction> {
  const roots = replyRoots(blocks);
  const parts: Array<{ interpretations: Interpretation[]; patterns: PatternConformance[] }> = [];
