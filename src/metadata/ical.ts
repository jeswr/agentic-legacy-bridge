...
export function unfoldIcalLines(text: string): string[] {
  ...
  for (const line of raw) {
    ...
    if (line !== "") out.push(line.slice(0, MAX_UNFOLDED_LINE_CHARS));
  }
  return out;
}
 ...
 */
export function parseIcalContentLine(line: string): IcalContentLine | undefined {
  ...
  for (let i = 0; i < line.length; i++) {
    ...
    else if (!inQuote && c === ";") semis.push(i);
    else if (!inQuote && c === ":") {
      valueStart = i;
      break;
    }
  ...
  const nameEnd = semis.length > 0 ? (semis[0] as number) : valueStart;
  ...
  for (let s = 0; s < semis.length && s < 32; s++) {
    const seg = line.slice((bounds[s] as number) + 1, bounds[s + 1] as number);
    ...
    if (key !== "" && key.length <= 64 && params[key] === undefined) params[key] = val;
  }
...
export function parseCalendar(text: string): ParsedCalendar {
  ...
  for (const rawLine of unfoldIcalLines(text)) {
    ...
    if (line.name === "BEGIN") {
      ...
      if (stack.length >= MAX_COMPONENT_DEPTH) break; // depth bomb → stop, keep what we have
      stack.push(comp);
    ...
    if (line.name === "END") {
      const comp = line.value.trim().toUpperCase();
      if (stack.length === 0 || stack[stack.length - 1] !== comp) break; // mismatched → stop
      stack.pop();
    ...
    if (stack.length === 1 && stack[0] === "VCALENDAR" && line.name === "METHOD") {
      if (method === undefined) method = line.value.trim().toUpperCase().slice(0, 32);
      continue;
    }
    if (current === undefined || stack[stack.length - 1] !== "VEVENT") continue;
 ...
 */
export function icalWhen(
  line: IcalContentLine,
): { when: NonNullable<ReturnType<typeof parseWhen>>; ambiguous: boolean } | undefined {
  ...
  const [, dpart, tpart, zulu] = m as unknown as [string, string, string, string];
  const iso =
    `${dpart.slice(0, 4)}-${dpart.slice(4, 6)}-${dpart.slice(6, 8)}` +
 ...
 */
export function extractCalendarInterpretations(
  parts: readonly string[] | undefined,
  ctx: InterpretContext,
): Interpretation[] {
  const out: Interpretation[] = [];
  if (parts === undefined || parts.length === 0) return out;
  let eventCount = 0;
  ...
  for (const part of parts) {
    ...
    for (const event of calendar.events) {
      if (eventCount >= MAX_VEVENTS) return out;
      eventCount++;
