...
import { htmlSafeJson, INLINE_CONTEXT, type MimePart, type ReplySigner } from "../reply.js";
 ...
 */
export async function buildActionMetadata(
  options: ActionMetadataOptions,
): Promise<BuiltActionMetadata> {
  const when = parseWhen(options.sentAt);
  if (when === undefined || when.kind !== "dateTime" || when.ambiguous) {
    throw new Error(
      "buildActionMetadata: sentAt must be a valid ISO-8601 datetime with an explicit timezone",
    );
  }
  ...
  if (derivedFrom !== undefined) subject.wasDerivedFrom = derivedFrom;
