  ...
  save(state: RelationshipState, expectedVersion?: string): Promise<void> {
    ...
    if (currentVersion !== expectedVersion) {
      return Promise.reject(
        new RelationshipConflictError(
          `relationship save conflict for ${state.personIri} (expected ${expectedVersion ?? "<new>"}, have ${currentVersion ?? "<new>"}).`,
        ),
      );
    }
 ...
 */
export function createPodRelationshipStore(
  options: PodRelationshipStoreOptions,
): RelationshipStore {
  ...
  if (!Number.isInteger(maxStateBytes) || maxStateBytes < 1 || maxStateBytes > MAX_STATE_BYTES) {
    throw new Error(
      `relationship store: maxStateBytes must be an integer from 1 to ${MAX_STATE_BYTES}.`,
    );
  }
  ...
  return {
    async load(personIri: string): Promise<LoadedRelationship | undefined> {
      ...
      if (state === undefined) return undefined; // unparseable → treat as no state (fail-closed)
    ...
    async save(state: RelationshipState, expectedVersion?: string): Promise<void> {
      ...
      else headers["if-none-match"] = "*"; // a brand-new resource must not already exist
 ...
 */
export function createGuardedUpgradeTransport(
  options: GuardedUpgradeTransportOptions = {},
): UpgradeTransport {
  ...
  if (
    ...
    throw new Error(
      `upgrade transport: maxResponseBytes must be an integer from 1 to ${MAX_RESPONSE_BYTES}.`,
    );
  }
...
export function encodeUpgradeOffer(offer: UpgradeOffer): Record<string, unknown> {
  return {
 ...
 */
export function decodeUpgradeResponse(raw: unknown): UpgradeResponse {
  if (typeof raw !== "object" || raw === null) return { accept: false };
...
function sanitizePeerHash(value: string): string {
...
) => Promise<{ readonly agentCardUrl: string } | undefined>;
...
export function recordBridgeDetected(
  store: RelationshipStore,
  personIri: string,
  now?: Date,
): Promise<TransitionResult> {
 ...
 */
export async function discoverCard(
  store: RelationshipStore,
  personIri: string,
  discover: CardDiscovery,
  now?: Date,
): Promise<TransitionResult> {
  const loaded = await store.load(personIri);
  if (loaded === undefined) return { ok: false, reason: "no relationship for counterparty." };
  ...
  if (found === undefined) return { ok: false, reason: "no bindable agent card discovered." };
  const result = transition(
    current,
    { kind: "card-discovered", agentCardUrl: found.agentCardUrl },
    now,
  );
  if (!result.ok) return result;
  await store.save(result.state, loaded.version);
 ...
 */
export async function offerAndNegotiate(
  store: RelationshipStore,
  personIri: string,
  offer: UpgradeOffer,
  ...
  if (normalized === undefined) return { ok: false, reason: "offer is malformed or unsafe." };

  let pending: LoadedRelationship;
  ...
  if (target === undefined) return { ok: false, reason: "no verified card endpoint to offer to." };
...
function sameOffer(left: UpgradeOffer | undefined, right: UpgradeOffer): boolean {
  return (
    left !== undefined &&
