...
function timestampToIso(ts: string): string | undefined {
 ...
 */
export function waIdToTelIri(waId: unknown): string | undefined {
  if (typeof waId !== "string") return undefined;
  const digits = waId.trim().replace(/^\+/, "");
  if (!/^[0-9]{1,20}$/.test(digits)) return undefined;
 ...
 */
function resolveMessages(envelope: Record<string, unknown>): {
  ...
  const collectValue = (value: Record<string, unknown> | undefined): void => {
    if (value === undefined) return;
    for (const m of asArray(value.messages) ?? []) {
      const rec = asRecord(m);
      if (rec !== undefined) messages.push(rec);
    }
    for (const c of asArray(value.contacts) ?? []) {
      const rec = asRecord(c);
      if (rec !== undefined) contacts.push(rec);
    }
  };
  ...
  if (entries !== undefined) {
    ...
    for (const entry of entries) {
      ...
      if (entryRec === undefined) continue;
      for (const change of asArray(entryRec.changes) ?? []) {
        const changeRec = asRecord(change);
        if (changeRec === undefined) continue;
        if (asString(changeRec.field) !== "messages") continue;
...
export function waMessageToBridgeMessage(
  ...
  const threadId =
    contextId !== undefined && WAMID.test(contextId) && contextId !== wamid ? contextId : undefined;

  // `timestamp` — Unix epoch SECONDS; best-effort, degrades to no date on a bad value.
  const timestamp = asString(inner.timestamp);
  const date =
    timestamp !== undefined && WA_TIMESTAMP.test(timestamp) ? timestampToIso(timestamp) : undefined;
...
export class WhatsAppChannelAdapter implements ChannelAdapter {
  ...
  parse(item: InboundRawMessage): BridgeMessage {
    return waMessageToBridgeMessage(
      item.raw,
      this.messageIndex !== undefined ? { messageIndex: this.messageIndex } : {},
    );
  }
